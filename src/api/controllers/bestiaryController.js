...
const getSpellsForArtifact = async (req, res) => {
  try {
    ...
    if (!element && !creatureType) {
      return res
        .status(HTTP_RESPONSES.OK)
        .json({ message: "Artifact with no spells" });
    }
    const query = {};
    if (element) query.element = element;
    if (creatureType) query.discipline = creatureType;
    if (spellType) query.type = spellType;
